...
from textbox.config.input import InputConfig
...
def get_yes_no(yes_no_prompt, io_config: InputConfig = None):
    ...
    while not parse.is_yes_no(input_str):
        io_config.input_function_parser(input_str)
        input_str = get_input(io_config.yes_no_fail_msg)
...
def get_str(prompt: str, prompt_end=":", default=None):
    ...
    while len(input_str) <= 0:
        input_str = get_input("No input was entered.")
        if input_str == "quit" or input_str == ":q":
            quit()
        if default is not None and len(input_str) == 0:
            input_str = default
...
#     opt_text = "<br>".join(
#         format_html_block("  ".join([f"#{option['key']}#: *{option['text']}*" for option in options]))
#     )
