...
def test_replace_character_at_end():
    """Test that replace_character at end of line doesn't cause IndexError"""
    # Test replacing character at the end position (length of text)
    text_line = TextLine("hello")
    text_line.replace_character("!", 5)  # At position 5 (length of "hello")
    assert text_line.text == "hello!"

    # Test replacing at various positions at the end
    text_line = TextLine("abc")
    text_line.replace_character("d", 3)  # Add at end
    assert text_line.text == "abcd"
    text_line.replace_character("e", 4)  # Add at new end
    assert text_line.text == "abcde"

    # Test replacing in the middle still works
    text_line = TextLine("hello")
    text_line.replace_character("a", 0)
    assert text_line.text == "aello"
    text_line.replace_character("!", 5)  # At end
    assert text_line.text == "aello!"
