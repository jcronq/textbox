...
def test_next_line():
    """Test that next_line property doesn't throw IndexError when on last line"""
    # Test empty text
    text = Text()
    ...
    # Test multiple lines, cursor on first line
    text = Text("hello\nworld")
    text.to_start_of_text()  # Go to first line
    assert text.next_line == TextLine("world")

    # Test multiple lines, cursor on last line (this was causing IndexError)
    text = Text("hello\nworld\nfoo")
    text.to_last_line()  # Move to last line
    assert text.next_line is None  # Should return None, not IndexError

    # Test three lines, move through them
    text = Text("line1\nline2\nline3")
    text.to_first_line()
    assert text.next_line == TextLine("line2")
    text.increment_line_ptr()
    assert text.next_line == TextLine("line3")
    text.increment_line_ptr()
    assert text.next_line is None  # On last line now
